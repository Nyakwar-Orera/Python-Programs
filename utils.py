...
def stable_log_sum(X):
    ...
    # compute the logarithm of the sum of the exponentials of the difference
    # between each element and the maximum value in each row
    log_sum = np.log(np.exp(X[:, 0] - max_val) + np.exp(X[:, 1] - max_val))

    # compute the sum of the logarithms and add the maximum value in each row
    result = np.sum(log_sum) + np.sum(max_val)
